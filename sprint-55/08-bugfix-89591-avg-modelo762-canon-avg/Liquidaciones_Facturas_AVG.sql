...
CREATE PROCEDURE [dbo].[Liquidaciones_Facturas_AVG]
...
;with aux as(
	...
	where 
			...
			AND (@contrato IS NULL  OR ctrcod= @contrato)
			AND (@version IS NULL OR ctrversion = @version)
	...
	select distinct
		...
		--***********************************************************************************************************
		--SYR-539161: Error: CFD9 - Según el tipo de factura, el campo 'Autoliquidación rectificada' es obligatorio
		--Necesitaremos conocer los nombres de fichero autoliquidación 2021S1 y 2021S2 -NÚMERO DEL FORMULARIO PRESENTADO- 
		--esos os los tendremos que pedir cada año y los meteremos a mano en los procedimientos de nuestra base de datos
		[modelo761Rectificada] = CASE WHEN f.facFecha IS NULL THEN NULL
									  WHEN YEAR(f.facFecha) = 2020 THEN IIF(MONTH (f.facFecha) <= 6, '7611000168030', '7611000164135')
									  WHEN YEAR(f.facFecha) = 2021 THEN IIF(MONTH (f.facFecha) <= 6	,'7612000004743', '7612000017763')
									  WHEN YEAR(f.facFecha) = 2022 THEN IIF(MONTH (f.facFecha) <= 6, '7612000031474', '7612000042211')
									  WHEN YEAR(f.facFecha) = 2023 THEN IIF(MONTH (f.facFecha) <= 6, '7612000050252', '7612000063875')
									  ELSE NULL END
		--***********************************************************************************************************
	...
	select
		...
		--Consumo Canon Fijo (20)
		CNS = (SELECT MAX(FL.fclunidades1 + FL.fclunidades2 + FL.fclunidades3 + FL.fclunidades4 + FL.fclunidades5 + FL.fclunidades6 + FL.fclunidades7 + FL.fclunidades8 + FL.fclunidades9)
			  FROM dbo.faclin  AS FL		
			  WHERE FL.fclFacCod= F.facCod AND FL.fclFacCtrCod= F.facCtrCod AND  FL.fclFacPerCod= F.FacPerCod AND FL.fclFacVersion= F.facVersion AND FL.fclTrfSvCod=20)
	...
	select 
		...
		--, consumoEstimado
		, [consumoAbastecimiento] =  CAST(CNS AS INT)
		, [consumoEstimado] =  CAST(CNS AS INT)
		, volumen, habitantes, ival, ccv, reduccion, cuotaFija, CV1, CV2, CV3, CVTotal, impDisp, cuotaTotal, indFact, periodo,
