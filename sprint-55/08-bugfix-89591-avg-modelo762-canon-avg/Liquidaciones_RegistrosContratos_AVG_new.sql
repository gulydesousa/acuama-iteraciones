...
ALTER PROCEDURE [dbo].[Liquidaciones_RegistrosContratos_AVG_nuevo]
...
BEGIN TRY
	...
	, FL_RN = ROW_NUMBER() OVER(PARTITION BY F.facCtrCod, F.facPercod, F.facCod, F.facVersion ORDER BY FL.fclNumLinea) 
	, NumOriginalesxCtr = 0
	, NumRectificativasConCanon = 0
	INTO #AUX
	...
	, NumOriginalesxCtr = SUM(IIF(facFechaRectif IS NULL, 1, 0)) 
	, NumRectificativasConCanon = SUM(IIF(Rectificada_TieneCanon=1, 1, 0)) 
	FROM #AUX AS A 
	WHERE FL_RN=1
	GROUP BY facCtrCod, facCtrVersion)
	...
	, A.ctrUso = C.ctrUso
	, NumOriginalesxCtr = ISNULL(I.NumOriginalesxCtr, 0)
	, NumRectificativasConCanon = ISNULL(I.NumRectificativasConCanon, 0)
	FROM #AUX AS A 
	LEFT JOIN F
	...
	LEFT JOIN INFO AS I
	ON I.facCtrCod = A.facCtrCod
	AND I.facCtrVersion = A.facCtrVersion;
