...
EXEC [InformesExcel].[AuditoresSevillaDeudaTipoFactura]  @p_params,  @p_errId_out OUTPUT, @p_errMsg_out OUTPUT;

SELECT @p_errMsg_out
*/

ALTER PROCEDURE [InformesExcel].[AuditoresSevillaDeudaTipoFactura]
...
AS
	...
	--**********************************************************************************
	--ADVERTENCIA: CODIGO REPETIDO!!!!
	--DESDE AQUI TODO EL CODIGO QUE VIENE DE [dbo].[Excel_ExcelConsultas.DeudaTipoFactura_EMMASA]
	--Como quieren que saque lo mismo, y lo de los Efectos Pendientes funciona a las bravas...
	...
	, ROW_NUMBER() OVER (PARTITION BY F.facCod , F.facPerCod, F.facCtrCod, F.facVersion ORDER BY CB.cobFec DESC, CB.cobFecReg DESC) AS RN
	FROM  #FACS AS F
	...
	, ROW_NUMBER() OVER (PARTITION BY R.facCod, R.facPerCod, R.facCtrCod, R.facVersion, R.efePdteCod ORDER BY CB.cobFec DESC,  CB.cobfecReg DESC) AS RN
	FROM #RECIBOS AS R
