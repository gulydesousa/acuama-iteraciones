...
SET @p_params= '<NodoXML><LI><FecDesde>20140701</FecDesde><FecHasta>20240630</FecHasta></LI></NodoXML>';/*
EXEC [dbo].[Excel_ExcelConsultas.DeudaTipoFactura_EMMASA_antes]  @p_params,  @p_errId_out OUTPUT, @p_errMsg_out OUTPUT;
EXEC [dbo].[Excel_ExcelConsultas.DeudaTipoFactura_EMMASA_antes]  @p_params,  @p_errId_out OUTPUT, @p_errMsg_out OUTPUT;
EXEC [dbo].[Excel_ExcelConsultas.DeudaTipoFactura_EMMASA_antes]  @p_params,  @p_errId_out OUTPUT, @p_errMsg_out OUTPUT;
...
CREATE PROCEDURE [dbo].[Excel_ExcelConsultas.DeudaTipoFactura_EMMASA_antes]
...
*/
	--**********************************************************************************
	--CODIGO REPETIDO!!!!
	--DESDE AQUI CODIGO QUE VIENE DE [dbo].[Excel_ExcelConsultas.DeudaTipoFactura_EMMASA]
	--Como quieren que saque lo mismo, y lo de arriba funciona a las bravas...
	--No ha quedado de otra. Lo siento :(
	--**********************************************************************************
	...
	*/
	...
	CREATE TABLE #ENTREGASCUENTA (
	[Tipo Factura] VARCHAR(10) NULL,
	Factura VARCHAR(12) collate  Modern_Spanish_CI_AS, 
	Uso VARCHAR(40) collate  Modern_Spanish_CI_AS, 
	facEstadoEmmasa VARCHAR(20) collate  Modern_Spanish_CI_AS,
	contrato INT,
	exentoCorte VARCHAR(1) collate  Modern_Spanish_CI_AS,
	estadoContrato VARCHAR(1) collate  Modern_Spanish_CI_AS,
	NIF VARCHAR(100) collate Modern_Spanish_CI_AS, 
	Nombre VARCHAR(200) collate  Modern_Spanish_CI_AS, 
	tipo VARCHAR(10) collate  Modern_Spanish_CI_AS,
	efePdteCod INT,
	estado VARCHAR(2) collate  Modern_Spanish_CI_AS,
	[Importe DEUDA] MONEY, 
	[Fecha Factura] DATE, 
	[Fecha VTO. Origen] DATE, 
	[Fecha Nuevo VTO] DATE	
	)
	...
	inner join coblin on cblPpag = cobPpag and cblNum = cobNum and cblScd = cobScd
	inner join vContratoUltVersion v1 on v1.ctrcod = cobCtr
	inner join usos u on v1.ctrUsoCod = usocod
	LEFT JOIN #EDOCTR AS CC
	ON CC.ctrcod = V1.ctrcod
	...
	cblPer = '999999'		
	group by v1.ctrTitDocIden, v1.ctrTitNom, cobFec, usodes, IIF(v1.ctrFicticio=1, NULL, v1.ctrcod), IIF(v1.ctrTitExtCorte=0, 'N', 'S'), CC.Estado
	HAVING SUM(cblImporte) <> 0;
	...
	AND F.facEstado NOT IN (4,5)		--Se excluyen las (4) AGRUPADAS y (5) Traspasadas. No es deuda 	
	...
	OR
	(--Facturas del ayuntamiento que son albaranes y la zona este cerrada
	--La fecha de cierre real de la zona este dentro del rango de fechas
	(C.ctrUsoCod = 5) AND
	(C.ctrGenAlb = 1) AND
	(PZ.przcierrereal IS NOT NULL) AND
	(P.fechaD IS NULL OR PZ.przcierrereal >= P.fechaD) AND
	(P.fechaH IS NULL OR PZ.przcierrereal < P.fechaH)
	)
	)
...
--	AND CBL.cblFacVersion = F.facVersion -- Es necesario sumar los cobros de todas las versiones de la factura para obtener la deuda real
	...
	--AND C.facVersion = R.facVersion -- Es necesario sumar los cobros de todas las versiones de la factura para obtener la deuda real
	LEFT JOIN UCOBS AS UC
	ON  UC.facCod = C.facCod
	AND UC.facPerCod = C.facPerCod
	AND UC.facCtrCod = C.facCtrCod;
	--AND UC.facVersion = C.facVersion; -- Es necesario sumar los cobros de todas las versiones de la factura para obtener la deuda real
	...
	WHERE E.cobEstado NOT IN ('CO', 'TR', 'CM')
	...
	, C.ctrFicticio
	, ISNULL(V.ctrTitExtCorte, 0) AS ctrTitExtCorte
	, C.ctrTitDocIden
	, C.ctrTitNom
	...
	ON C.ctrcod = F.facCtrCod
	AND C.ctrversion = F.facCtrVersion
	LEFT JOIN dbo.vContratoUltVersion AS V 
	...
	SELECT Año 
	, facFecha
	, serdesc AS [Tipo Factura]
	, facNumeroAqua AS [Factura]
	, CASE WHEN usodes is null THEN '*************' ELSE usodes END AS Uso
	, facEstadoEmmasa AS [Estado Factura]
	, IIF(ctrFicticio=1, NULL, facCtrcod) AS [Contrato]
	, IIF(ctrTitExtCorte=0, 'N', 'S') AS [Exento Corte]
	, ctrEstado AS [Estado Contrato]
	, ctrTitDocIden AS [NIF]
	, ctrTitNom AS [Nombre]
	,IIF(deuda < 0, 'DEVOLUCION', 'PAGO') AS Tipo
	, efePdteCod AS [Efecto Pendiente] 
	, cobEstado AS [Estado Cobro]
	--, facturado AS [Importe Facturado]
	, deuda * IIF(cobEstado IS NOT NULL AND cobEstado='PD', -1, 1) AS [Importe DEUDA]	
	, CONVERT(DATE,facFecha) AS [Fecha Creación]
	, CONVERT(DATE,facFechaVtoOrig) AS [Fecha VTO. Origen]
	, IIF(r.efePdteCod IS NULL, CONVERT(DATE,facFechaVto), CONVERT(DATE, r.efePdteFecVencimiento)) AS [Fecha Nuevo VTO]	
	, [Importe PAGO] = deuda*IIF(cobEstado IS NOT NULL AND cobEstado='PD', 0, 1)
	, [Importe DEVOLUCION] =  deuda* IIF(cobEstado IS NOT NULL AND cobEstado='PD', -1, 0)
	FROM #REPORT r
	UNION ALL
	SELECT 
	YEAR([Fecha Factura]),
	...
	[Fecha Nuevo VTO]
	, [Importe PAGO] = 0
	, [Importe DEVOLUCION] =  [Importe DEUDA]* -1
	FROM #ENTREGASCUENTA)

	--**********************************************************************************
	--CODIGO REPETIDO!!!!
	--HASTA AQUI CODIGO QUE VIENE DE [dbo].[Excel_ExcelConsultas.DeudaTipoFactura_EMMASA]
	--Como quieren que saque lo mismo, y lo de arriba funciona a las bravas. 
	--No ha quedado de otra. Lo siento :(
	--**********************************************************************************	
	SELECT NIF
	, Nombre
	, Factura
	, [Fecha Factura] = CAST(facFecha as date)
	, [Estado]	= D.[Estado Cobro]
	, D.[Importe PAGO] 
	, D.[Importe DEVOLUCION]
	, D.[Importe DEUDA]
	, D.[Efecto Pendiente]
	FROM DATOS AS D
	WHERE D.[Estado Cobro] NOT IN('CO', 'TR', 'CM', 'FR')
	--Algun importe debe tener
	AND (D.[Importe DEVOLUCION]<>0 OR D.[Importe PAGO] <>0 OR D.[Importe DEUDA]<>0);	
