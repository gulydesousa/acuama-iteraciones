USE [ACUAMA_EMMASA_DEVELOP]
GO

/****** Object:  StoredProcedure [dbo].[Excel_ExcelConsultas.DeudaAuditoresSevilla_EMMASA]    Script Date: 17/06/2024 13:28:02 ******/
SET ANSI_NULLS ON
GO
...
SET @p_params= '<NodoXML><LI><FecDesde>20110301</FecDesde><FecHasta>20210414</FecHasta></LI></NodoXML>';
...
AS
	...
	select v1.ctrTitDocIden AS [NIF], v1.ctrTitNom AS Nombre, NULL AS Factura, 
		NULL AS [Fecha Factura], --CONVERT(DATE,cobFec) AS [Fecha Factura], 
		'PD' AS Estado, 0 AS [Importe PAGO],  SUM(cblImporte) AS [Importe DEVOLUCION] 	
	from cobros C
	inner join coblin on cblPpag = cobPpag and cblNum = cobNum and cblScd = cobScd
	inner join vContratoUltVersion v1 on v1.ctrcod = cobCtr
