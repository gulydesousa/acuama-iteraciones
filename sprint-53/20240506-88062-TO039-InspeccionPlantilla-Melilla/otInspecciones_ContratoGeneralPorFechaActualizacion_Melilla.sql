...
ALTER PROCEDURE [InformesExcel].[otInspecciones_ContratoGeneralPorFechaActualizacion_Melilla]
...
AS
	...
	BEGIN
		...
		WITHIN GROUP (ORDER BY V.otivOrden, v.otivDescripcion DESC)
		...
		*/
		...
		SELECT @CRITICAS = STUFF((
		SELECT ',' + otivColumna
		FROM dbo.otInspeccionesValidaciones AS V
		WHERE otivCritica = 1 AND otivServicioCod = @svcCod
		ORDER BY otivOrden, otivDescripcion DESC
		FOR XML PATH(''), TYPE
		).value('.', 'NVARCHAR(MAX)'), 1, 1, '');

		SELECT @NO_CRITICAS = STUFF((
		SELECT ',' + otivColumna
		FROM dbo.otInspeccionesValidaciones AS V
		WHERE otivCritica = 0 AND otivServicioCod = @svcCod
		ORDER BY otivOrden, otivDescripcion DESC
