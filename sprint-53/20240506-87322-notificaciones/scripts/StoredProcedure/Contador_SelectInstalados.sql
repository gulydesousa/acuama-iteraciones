/*
	...
	--Mejoras Generación masiva de ordenes de trabajo Acuama
  ...
  , @SoloConPlazoNotificacion BIT = 0
  ...
  EXEC [dbo].[Contador_SelectInstalados] @incidenciaCambioContador, @codigo, @inciLecLectorD, @inciLecLectorH, @inciLecInspD, @inciLecInspH, @contratoD, @contratoH, @contadorD, @contadorH, @fechaCompraD
										...
										, @SoloPendientes, @SoloInspeccionesAptas, @SoloConPlazoNotificacion;
...
ALTER PROCEDURE [dbo].[Contador_SelectInstalados] 
  ...
  , @SoloPendientes BIT = NULL
  , @SoloInspeccionesAptas BIT = NULL
  , @SoloConPlazoNotificacion BIT = NULL
AS
SET NOCOUNT ON;
...
LEFT JOIN dbo.vOtInspeccionesNotificacionEdo_Melilla AS V
ON V.contrato = C.ctrcod 
WHERE (
		  ...
		  (@SoloConPlazoNotificacion IS NULL OR @SoloConPlazoNotificacion=0 OR (@SoloConPlazoNotificacion= 1 AND V.FinalPlazo<=GETDATE())) AND
