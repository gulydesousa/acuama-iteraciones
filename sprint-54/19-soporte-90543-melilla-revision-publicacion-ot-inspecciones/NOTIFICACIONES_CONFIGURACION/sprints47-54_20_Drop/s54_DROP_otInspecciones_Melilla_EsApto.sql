--USE ACUAMA_DESA
--DROP PROCEDURE dbo.otInspecciones_Melilla_EsApto;
--GO
...
AS
	...
	--Tabla donde guardamos las columnas que se usan para evaluar si es apto o no
	DECLARE @DATOS_EVAL AS TABLE(Clave VARCHAR(128), Valor VARCHAR(250) DEFAULT '', esOK VARCHAR(25) DEFAULT'NO');
	...
	BEGIN
		...
		--SELECT @otivColumnas, @columns;

		-- Construir la cadena de consulta SQL	
		SET @sql = CONCAT(
		'SELECT Clave, Valor, dbo.fOtInspecciones_Melilla_Valor(Clave, Valor) ',
		'FROM (SELECT ', 
		@otivColumnas, 
		' FROM dbo.otInspecciones_Melilla AS I WHERE objectid=', 
		@objectid, 
		') AS SourceTable UNPIVOT (Valor FOR Clave IN (', 
		@columns, 
		')) AS UnpivotTable');

		INSERT INTO @DATOS_EVAL (Clave, Valor, esOK)
