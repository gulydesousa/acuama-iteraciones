...
, agrupadas as(
	select tipo, uso, facNumero, facNumeroRectif, contrato, TitDocIden, fecInicio, fecFin, diasF1, diasF2, consumoAbastecimiento, consumoEstimado, volumen, 
	 ...
	 from aux
)--SELECT * FROM agrupadas WHERE contrato=7449
...
, agrupUnificadas as(
	...
	from agrupadas
	where RN = 1 AND contrato=7449
)--SELECT * FROM agrupUnificadas WHERE contrato=7449

, conRectificadas as(
	select a.contrato, a.periodo
		...
		--Necesitaremos conocer los nombres de fichero autoliquidación 2021S1 y 2021S2, 
		...
		[modelo761Rectificada] = CASE WHEN f.facFecha IS NULL THEN NULL
									  WHEN YEAR(f.facFecha) = 2020 AND MONTH (f.facFecha) <= 6	THEN '7611000168030'
									  WHEN YEAR(f.facFecha) = 2020 AND MONTH (f.facFecha) > 6	THEN '7611000164135'
									  --***
									  WHEN YEAR(f.facFecha) = 2021 AND MONTH (f.facFecha) <= 6	THEN '7612000004743'
									  WHEN YEAR(f.facFecha) = 2021 AND MONTH (f.facFecha) > 6	THEN '7612000017763'
									  --***
									  WHEN YEAR(f.facFecha) = 2023 AND MONTH (f.facFecha) <= 6	THEN '2023S1'
									  WHEN YEAR(f.facFecha) = 2023 AND MONTH (f.facFecha) > 6	THEN '2023S2'
									  ELSE NULL END
		--***********************************************************************************************************
		*/
...
SELECT * FROM facturas WHERE facCtrCod=7449 AND facPerCod LIKE '2023%'
/*, conFugas as(
	select
		r.*,
