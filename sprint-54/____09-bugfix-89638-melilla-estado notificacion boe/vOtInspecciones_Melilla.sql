--SELECT * FROM [dbo].[vOtInspecciones_Melilla] 
...
SELECT [CONTRATO GENERAL], [CONTRATO ABONADO], [INSPECCION], [MOCK] = 0 
FROM otInspeccionesContratos_Melilla
--Trucamos metiendo en el listado de contratos hijos los padres que faltan, asi siempre tenemos el padre con sus hijos
UNION ALL
SELECT [CONTRATO GENERAL], [CONTRATO GENERAL], [INSPECCION], [MOCK]= 1 
FROM PADRES
...
, RN_ABONADOS = ROW_NUMBER() OVER (PARTITION BY I.objectid ORDER BY  IIF(C.[CONTRATO ABONADO]=C.[CONTRATO GENERAL], 0, 1), C.[CONTRATO ABONADO])
--NUM_ABONADOS: Total de abonados
, NUM_ABONADOS = SUM(IIF(C.[CONTRATO ABONADO]=C.[CONTRATO GENERAL], 0, 1)) OVER (PARTITION BY objectid)
, MOCK = ISNULL(C.MOCK, 0)
...
, RefIntBOE, NumBOE
--EdoRN=1: Para la entrega mas reciente de cualquiera de los duplicados
, EdoRN = ROW_NUMBER() OVER (PARTITION BY  I.objectid, I. ctrcod 
		  ORDER BY IIF(E.NotificadoFecha IS NULL, 1, 0), E.NotificadoFecha)
FROM INSP AS I
LEFT JOIN dbo.vOtInspeccionesNotificacionEdo_Melilla AS E
ON E.contrato =  I.ctrcod
