...
*/
	...
	ON OT.otCtrCod = C.ctrcod
	INNER JOIN dbo.contadorCambio AS CC
	ON OT.otsercod = CC.conCamOtSerCod
	AND OT.otserscd = CC.conCamOtSerScd
	AND OT.otnum = CC.conCamOtNum
	INNER JOIN dbo.contador AS CO
	ON CO.conID = CC.conCamConID
	LEFT JOIN dbo.modcon AS MO
	ON MO.mdlCod = CO.conMdlCod
	AND MO.mdlMcnCod = CO.conMcnCod
