...
WHERE T.trfFecReg = @fechareg --Tarifas del excel del syrena dadas de alta
AND T.trfUsrReg = 'gmdesousa';
/*
INSERT INTO @TRFBAJA
SELECT trfsrvcod, trfcod,  NULL
FROM dbo.tarifas AS T 
--INNER JOIN dbo.tarval AS TV
--ON T.trfsrvcod = TV.trvsrvcod
--AND T.trfcod = TV.trvtrfcod
WHERE T.trfFecUltMod = @fechareg --Tarifas que hay que se han dado de baja
