...
BEGIN TRY	
    ...
    FROM vOtInspeccionesNotificacionEmisiones_Melilla  AS V 
		INNER JOIN vOtInspecciones_Melilla AS VV
		ON V.objectid = VV.objectid
    ...
    WHERE  (@excluirNoEmision=0 OR (@excluirNoEmision=1 AND (CC.ctrNoEmision IS NULL OR CC.ctrNoEmision=0)))
        AND (@contratoD IS NULL OR VV.ctrcod>=@contratoD)
        AND (@contratoH IS NULL OR VV.ctrcod<=@contratoH)
